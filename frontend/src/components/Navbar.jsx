...
export default function Navbar() {
  ...
  return (
    <nav className={`w-full border-b backdrop-blur-md sticky top-0 z-50 px-6 py-3.5 flex items-center justify-between transition-colors duration-300 ${
      isDark ? 'border-[#231E33] bg-[#0A0A0F]/90 text-[#FAFAFA]' : 'border-slate-200 bg-white/90 text-slate-900 shadow-sm'
    }`}>
      ...
      <div className="flex items-center gap-6 relative" ref={menuRef}>
        ...
        <Link to="/" className={`flex items-center gap-3 font-bold text-xl tracking-tight group ${isDark ? 'text-[#FAFAFA]' : 'text-slate-900'}`}>
          <div className="w-9 h-9 rounded-xl flex items-center justify-center p-1 overflow-hidden transition-all duration-300 group-hover:scale-105 bg-gradient-to-br from-[#1A1528] to-[#0A0A0F] border border-[#8B5CF6]/35 shadow-[0_0_15px_rgba(139,92,246,0.3)] group-hover:border-[#8B5CF6]">
            <img 
              src="/logo.png" 
              alt="WebShield AI Logo" 
              className="w-full h-full object-contain drop-shadow-[0_0_8px_rgba(139,92,246,0.8)]" 
            />
          </div>
        ...
        <div className={`hidden lg:flex items-center gap-1 pl-4 border-l ${isDark ? 'border-[#231E33]' : 'border-slate-200'}`}>
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer ${
              isDark ? 'text-neutral-300 hover:text-white hover:bg-[#1A1528]' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
            }`}
          >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={profileRef}>
          {currentUser ? (
            ...
            <Link 
              to="/login" 
              className="bg-gradient-to-r from-[#8B5CF6] to-[#EC4899] hover:opacity-95 text-white font-semibold px-4 py-2 rounded-xl transition flex items-center gap-2 text-sm shadow-md shadow-purple-950/40"
            >
