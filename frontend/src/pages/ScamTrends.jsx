...
export default function ScanTrends() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-73px)] w-full flex flex-col items-center px-4 sm:px-8 lg:px-12 py-10 bg-[#0A0A0F] text-[#FAFAFA] font-sans">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-[#8B5CF6]/10 rounded-full blur-[180px] pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-6xl flex items-center justify-between mb-8">
        ...
        >
          <ArrowLeft className="w-4 h-4 text-[#8B5CF6]" /> Return Back
        </button>
      ...
      <div className="relative z-10 w-full max-w-6xl space-y-6">
        ...
        <div className="w-full bg-[#13111C] border border-[#231E33] rounded-3xl p-6 sm:p-8 shadow-xl flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#1A1528] border border-[#231E33] text-purple-300 text-xs font-mono uppercase">
              <TrendingUp className="w-3.5 h-3.5 text-[#8B5CF6]" /> Threat Intelligence Core
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-[#FAFAFA] tracking-tight">Global Scam & Phishing Trends</h1>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto justify-between lg:justify-end">
            <div className="p-4 rounded-xl bg-[#0A0A0F] border border-[#231E33] text-center min-w-[140px]">
              <p className="text-[10px] font-mono text-neutral-500 uppercase tracking-wider">Indexed Scans</p>
              <p className="text-xl font-bold text-[#8B5CF6] mt-1">142,850+</p>
            </div>
            <button 
              onClick={handleExportReport}
              className="flex items-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-[#8B5CF6] to-[#EC4899] hover:opacity-95 text-white text-xs font-bold transition shadow-lg shadow-purple-950/50 cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-[#13111C] border border-[#231E33] rounded-2xl p-5 flex items-center gap-4 shadow-md">
            <div className="p-3 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
              <ShieldCheck className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs font-mono text-neutral-400 uppercase tracking-wider">Safe Link Accuracy</p>
              <p className="text-xl font-bold text-[#FAFAFA] mt-0.5">88.4%</p>
            </div>
          </div>
          ...
          <div className="bg-[#13111C] border border-[#231E33] rounded-2xl p-5 flex items-center gap-4 shadow-md">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-[#FAFAFA] mt-0.5">11.6%</p>
            </div>
          </div>
          ...
          <div className="bg-[#13111C] border border-[#231E33] rounded-2xl p-5 flex items-center gap-4 shadow-md">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-[#FAFAFA] mt-0.5">&lt; 0.42s</p>
            </div>
          </div>
        </div>

        {/* Threat Vector Breakdown Panel */}
        <div className="bg-[#13111C] border border-[#231E33] rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-40 sm:w-48 h-9 pl-8 pr-3 bg-[#0A0A0F] border border-[#231E33] rounded-lg text-xs text-[#FAFAFA] outline-none focus:border-[#8B5CF6] transition"
                />
              </div>
              <select 
                value={selectedFilter}
                onChange={(e) => setSelectedFilter(e.target.value)}
                className="h-9 px-3 bg-[#0A0A0F] border border-[#231E33] rounded-lg text-xs text-neutral-300 outline-none focus:border-[#8B5CF6] transition cursor-pointer"
              >
                <option value="all" className="bg-[#13111C]">All Risk Levels</option>
                <option value="critical" className="bg-[#13111C]">Critical</option>
                <option value="high" className="bg-[#13111C]">High</option>
                <option value="medium" className="bg-[#13111C]">Medium</option>
              </select>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {filteredThreats.length > 0 ? filteredThreats.map((threat, index) => (
              <div key={threat.id} className="p-4 sm:p-5 bg-[#0A0A0F] border border-[#231E33] rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-neutral-700 transition">
                <div className="flex items-start sm:items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[#13111C] border border-[#231E33] flex items-center justify-center text-xs font-mono font-bold text-neutral-400 shrink-0">
                    {threat.id}
                  </div>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <h3 className="text-xs sm:text-sm font-bold text-[#FAFAFA]">{threat.title}</h3>
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono uppercase bg-[#1A1528] text-neutral-400">{threat.category}</span>
                    </div>
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-[#231E33]">
                  ...
                  <div className="text-right">
                    <span className="text-sm sm:text-base font-extrabold text-[#FAFAFA] font-mono">{threat.percentage}</span>
