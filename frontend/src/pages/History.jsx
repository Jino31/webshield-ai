...
export default function History() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-73px)] w-full flex flex-col items-center px-4 sm:px-8 lg:px-12 py-12 bg-[#07090E] text-[#FAFAFA]">
      {/* Background VFX */}
      ...
      <div className="absolute top-10 left-1/4 w-[400px] h-[400px] bg-cyan-500/10 rounded-full blur-[140px] pointer-events-none" />

      {/* Header with Back Button */}
      <div className="relative z-10 w-full max-w-4xl flex items-center justify-between mb-8">
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-2 text-xs font-medium text-slate-300 hover:text-white bg-[#0C1220] border border-[#16223A] px-4 py-2.5 rounded-xl transition shadow-sm cursor-pointer"
        >
          <ArrowLeft className="w-4 h-4 text-cyan-400" /> Back
        </button>
        <div className="flex items-center gap-2 text-slate-400 text-xs font-mono">
          <HistoryIcon className="w-4 h-4 text-cyan-400" /> Scan Audit Logs
        </div>
      </div>

      {/* History Content Card */}
      <div className="relative z-10 w-full max-w-4xl bg-[#0C1220]/90 backdrop-blur-2xl border border-[#16223A] rounded-3xl p-6 sm:p-8 shadow-2xl shadow-cyan-950/40">
        <div className="border-b border-[#16223A] pb-5 mb-6">
          <h2 className="text-2xl font-bold tracking-tight text-white">Recent Scan History</h2>
          <p className="text-slate-400 text-xs sm:text-sm mt-1">Review previously analyzed URLs and classification verdicts.</p>
        </div>

        <div className="space-y-3">
          {scanHistory.map((item) => (
            <div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-[#080D1A] border border-[#16223A] hover:border-cyan-500/50 p-4 rounded-2xl transition gap-3">
              <div className="flex items-center gap-3.5 overflow-hidden">
                ...
                <div className="overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-500 mt-0.5">
                    ...
                    <span>•</span>
                    <span>Confidence: <strong className="text-cyan-300 font-mono">{item.score}%</strong></span>
                  </div>
                </div>
              </div>
